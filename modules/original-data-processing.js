...
const step_01 = (input) => {
    return new Promise((resolve, reject) => {
        separator('🚗🚗🚗「step_01」is processing');
        exec(`wc -l ${input}`, function (error, results) {
            const total = Number(results.replace(input, '').trim());
            const output = input.replace(path.basename(input), `[step_01]${path.basename(input)}`).replace(path.extname(input), '.node');
            if (total && total > 0) {
                let index = 0;
